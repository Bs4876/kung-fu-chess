...
A single-process WebSocket server that lets two real clients (or a client
and a trivial bot) play a game of Kung Fu Chess over a real network
connection, instead of one process sharing an in-process `GameEngine`
directly. This is a second entrypoint alongside `server/main.py`'s
...
- **`net/matchmaking.py`** / **`net/room_registry.py`** - the two ways a
  game actually gets created: automatic ELO-range pairing (falling back to
  a bot after a wait), or a manually created/joined room. Both terminate in
  an identical `GameRoom`, built from the same injected factory.
- **`net/bot_player.py`** - a trivial stand-in opponent: one random legal
  move per tick, submitted through the same path a real player's move
  takes.
...
matchmaking, disconnect/forfeit, ELO updates, bot fallback, and rooms
together against one running server instance.
