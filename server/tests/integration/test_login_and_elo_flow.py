...
async def test_two_logged_in_users_play_to_king_capture_and_both_elo_ratings_update(tmp_path):
    ...
    assert handle_register({"username": "alice", "password": "pw"}, alice_session, users)["success"]
    assert handle_register({"username": "bob", "password": "pw"}, bob_session, users)["success"]
    assert handle_login({"username": "alice", "password": "pw"}, alice_session, users)["success"]
    assert handle_login({"username": "bob", "password": "pw"}, bob_session, users)["success"]
