...
which also translates GameEngine.wait()'s resolved outcomes into the
state/game_events.py types log/score panels subscribe to. GameFacade itself
just coordinates: the engine, the tracker, and the event stream.
"""
...
class GameFacade:
    ...
    def tick(self, dt_ms: int):
        ...
        for event in self._motions.resolve(outcomes):
